...
- 已知限制：当前没有 match generation 机制，也不会在开局或返回菜单时取消挂起的 AI 协程；若玩家在 `_run_ai_turn()` 计时器到期前退出并立即开始新对局，旧回调可能在新牌桌上醒来并直接行动。行动仍经 `PokerRound.apply_action()` 校验，后果限于 AI 在新对局中提前行动，但后续应在开局/返回菜单时作废挂起回调（例如引入 match 代际标记）。当前 UI 探针固定 `ai_pending` 并同步执行，未覆盖该竞态。
