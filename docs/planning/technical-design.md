...
> 范围说明：本文保留原型/首发架构基线。教程、配置、牌谱回放、统计/成就、AI 策略、教练与画像的现行机制以[架构说明](../architecture.md)为准。本文对旧 profile 的描述不限制已实现的独立牌谱存储；保存已结束手牌不等于保存进行中的权威牌局。
...
- `scripts/ai/` 内的起手范围、公开行动推断、Monte Carlo、有限行动收益评估、个性配置和行动选择；现行算法边界与验收见[架构](../architecture.md#ai-layer)和[测试计划](test-plan.md#算法与教练回归)。
...
| AI 推进服从可见 UI 生命周期 | 日志、设置和暂停期间不能让延迟回调偷偷改变牌局。 | `_ai_can_advance()` 同时检查牌桌存在、暂停、日志和弹窗状态，延迟结束后执行前再次检查；回到菜单时旧回调因牌桌不存在而终止。 |
...
6. AI 回合在独立快照上后台计算，同时累计行动延迟；日志、可见弹窗和暂停状态阻止应用结果并冻结剩余等待时间。
7. 延迟结束且结果就绪后，核对对局代际、手牌、行动者和街道，合法结果经 `PokerRound.apply_action()` 应用；旧结果丢弃。
...
- 暂停和日志状态属于 UI 生命周期，不修改规则引擎；从暂停覆盖层返回菜单后，待执行 AI 回调会因牌桌不存在而直接终止。
...
| 已确定的逐手历史如何持久化；是否另行支持当前牌局续玩 | 新规划确定完结手牌回放，需要版本、幂等和旧数据迁移；当前牌局续玩未纳入已确定范围。 |
| 最终中文字体、动画和高 DPI 策略 | 会影响资产授权、节点结构、性能与平台适配。 |
| 是否切换到非 Mono Godot | 当前本机用 Mono app 运行；项目本身不使用 C#，但工具链选择会影响本地验证说明。 |

## 首发补充契约（2026-09-07）

首发完成定义见 [release-plan.md](release-plan.md)。线程计算仅拥有快照；主线程统一提交玩家与 AI 行动。所有长期动画绑定可见节点生命周期，不依赖仍存在的主场景来间接清理。存档字段先按类型校验，临时文件写入成功后才替换旧文件；保存失败必须提示。运行代码不增加外部服务。

测试脚本可固定 shuffle RNG，并使用独立 profile 路径；UI 脚本探针不再清空玩家战绩。标准构建固定 Godot 4.7.2 与对应官方模板；独立包自检不得依赖编辑器、外部脚本注入或源码路径。现行代码机制以 [architecture.md](../architecture.md) 为准。
