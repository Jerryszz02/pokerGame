...
> 范围说明（2026-09-10）：本文保留原牌桌 UI 的验收基线；模式、教程、配置与回放交互由文末练习页面探针和[测试计划](test-plan.md#练习产品回归2026-09-10)覆盖。下文“只修改 UI”的步骤限于原 UI 修复任务，不限制新规划中必要的规则配置和持久化实现。
...
新增 `practice_ui_probe.gd` 在三档窗口下操作模式入口、草稿取消、四档筹码/盲注、参考覆盖层、真实回放、统计筛选和课程完成。窗口模式截图位于 `/tmp/poker_practice_audit/`。滚动内容允许超出滚动区域，其容器和固定关闭按钮仍须在视口内；不可用的分析面板不得展示伪造建议。已有牌桌验收继续有效。执行命令见 [Runbook](../runbook.md)，结果以对应提交的日志和[发布记录](../releases/1.3.0.md#verification-and-limits)为准。

## 三手牌桌教程（2026-09-23）

`guided_tutorial_ui_probe.gd` 从首页直接进入真实牌桌，操作前两手的继续、跟注、让牌和弃牌，验证第三手加注、全下确认及输赢分支。覆盖观察阶段冻结、重复点击、旧 AI 结果失效、设置及语言切换、保存失败重试和普通战绩隔离。以 1280×720、1440×900、1920×1080 检查中英文教练卡片不遮挡底牌、公共牌、底池或操作区，摊牌高亮来自评估器的最佳五张。窗口截图写入 `/tmp/poker_guided_tutorial_audit/`，已接入 `verify.py --windowed`。
