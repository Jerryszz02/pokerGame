...
新增 `practice_ui_probe.gd` 在三档窗口下操作模式入口、草稿取消、四档筹码/盲注、参考覆盖层、真实回放、统计筛选和课程完成。窗口模式截图位于 `/tmp/poker_practice_audit/`。滚动内容允许超出滚动区域，其容器和固定关闭按钮仍须在视口内；不可用的分析面板不得展示伪造建议。已有牌桌验收继续有效。实际结果见 [实施记录](practice-implementation.md)。
