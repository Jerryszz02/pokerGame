...
当前结论：工程修复与候选构建已取得验证证据，尚未满足稳定公开首发。目标与放行标准保持 [R1–R9](../planning/release-plan.md)，不因已有测试通过而降低目标设备、真人试玩或公开下载要求。
...
- 平台/渠道仍按建议准备 Windows x64 + macOS / GitHub Releases，待用户固定最终首发范围。CI 的 Windows Server 环境不等同于已验证 Windows 消费级桌面最低要求。
...
| R4 理解/操作 | 帮助与离桌确认、偏好类型恢复与写入失败回归；可见音效/节奏开关调用实际行为并保存；保存失败弹窗已经截图审查。 | 软件行为检查通过，真人理解与实际设备音效仍需 R7/R8。 |
...
| R7 目标平台 | macOS 从非源码目录打开真实程序、检查启动菜单；实际包在禁止联网的进程沙箱中完成 9 组自检。签名完整性检查退出 0，Gatekeeper 退出 3 / rejected。 | 未通过：Windows 图形设备、完整目标设备流程、macOS 正式签名/下载隔离体验缺证据。 |
| R8 真人试玩 | 已准备[匿名记录模板](playtest-template.md)。 | 0/2 份已收到反馈；每位至少 20 分钟，至少一位不熟悉项目。 |
...
需要固定目标平台/渠道，提供可用 Windows 图形设备和 macOS 签名/公证条件，并收到两份真实试玩记录。随后按反馈修正、重验受影响项，完成最终提交的 CI/必要审查，受控 squash 合并，公开发布并重新下载验收。凭据不进入仓库或试玩记录。
