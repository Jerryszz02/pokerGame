...
func _probe(viewport: Vector2i) -> void:
	...
	var personality: OptionButton = scene.find_child("OpponentPersonalityOptions",true,false)
	var personality_group: Control = scene.find_child("OpponentPersonalityGroup",true,false)
	check(personality != null and personality_group != null,"whole-table personality selector exists")
	check(personality_group.visible,"personality selector visible for medium")
	var rock_index: int = MatchConfig.OPPONENT_PERSONALITIES.find("Rock")
	personality.select(rock_index)
	personality.item_selected.emit(rock_index)
	scene.difficulty_options.select(0)
	scene.difficulty_options.item_selected.emit(0)
	check(scene.pending_match_config.difficulty == "simple" and not personality_group.visible,"personality selector hidden for simple")
	scene.difficulty_options.select(2)
	scene.difficulty_options.item_selected.emit(2)
	check(personality_group.visible and personality.selected == rock_index,"remembered selection survives simple and restores on hard")
	scene.difficulty_options.select(1)
	scene.difficulty_options.item_selected.emit(1)
	check(personality_group.visible and personality.selected == rock_index,"medium restores the remembered selection")
	...
	check(scene.game.match_config.opponent_personality == "Rock","visible personality selection flows into the match config")
	for i in range(1,scene.game.players.size()):
		check(str(scene.game.players[i].personality.get("name","")) == "Rock","named preset reaches every AI seat")
	check(scene.LocalProfileScript.load_profile(scene.profile_path).settings.opponent_personality == "Rock","personality persists across launch")
