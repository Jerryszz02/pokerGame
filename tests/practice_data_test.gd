...
func _test_guided_store() -> void:
	var path := scratch.path_join("guided-legacy")
	var original := PracticeStore.new(path)
	var game := _game()
	_passive(game)
	var record := game.completed_hand_record()
	check(original.commit_hand(record) and original.complete_tutorial("T1"), "legacy lesson and ordinary hand setup")
	var legacy := original.state.duplicate(true)
	legacy.erase("guided_tutorial")
	legacy.achievements.first_lesson = {"name": "旧课程奖章", "source": "教程"}
	_write(path.path_join("profile.json"), JSON.stringify(legacy))
	var store := PracticeStore.new(path)
	check(store.notice.is_empty() and store.guided_resume_lesson() == "G1", "old profile loads with empty guided progress")
	check(store.state.tutorial_completed == ["T1"] and store.state.achievements.first_lesson.name == "旧课程奖章", "old lesson and award retained")
	check(store.records().size() == 1 and store.statistics().hands == 1, "old replay and stats retained")
	check(store.complete_guided_tutorial("G2"), "guided progress accepts valid out-of-order replay")
	check(store.guided_resume_lesson() == "G1" and store.state.guided_tutorial.completed == ["G2"], "resume selects first unfinished hand")
	check(store.complete_guided_tutorial("G1"), "old profile remains writable")
	check(store.state.achievements.first_lesson.name == "旧课程奖章", "guided lesson preserves prior award content")
	var bytes := FileAccess.get_file_as_string(path.path_join("profile.json"))
	check(store.complete_guided_tutorial("G1") and FileAccess.get_file_as_string(path.path_join("profile.json")) == bytes, "guided completion is idempotent")
	check(not store.complete_guided_tutorial("G4") and store.state.guided_tutorial.completed.size() == 2, "invalid guided id rejected")
	check(store.complete_guided_tutorial("G3") and store.guided_resume_lesson() == "G1", "all guided hands complete and restart offered")
	check(store.state.achievements.all_lessons.name == "完成入门课程", "three hands unlock existing completion award")
	var reopened := PracticeStore.new(path)
	check(reopened.state.guided_tutorial.completed == ["G2", "G1", "G3"], "guided progress survives restart")
	check(reopened.state.tutorial_completed == ["T1"] and reopened.statistics().hands == 1, "legacy progress and ordinary stats survive guided saves")

	for invalid in [{"version": 2, "completed": []}, {"version": 1, "completed": ["G1", "G1"]}, {"version": 1, "completed": ["G4"]}, {"version": 1, "completed": "G1"}]:
		var bad_path := scratch.path_join("guided-invalid-" + str(invalid.hash()))
		var bad := legacy.duplicate(true)
		bad.guided_tutorial = invalid
		var bad_bytes := JSON.stringify(bad)
		_write(bad_path.path_join("profile.json"), bad_bytes)
		var locked := PracticeStore.new(bad_path)
		check(not locked.complete_guided_tutorial("G1") and not locked.commit_hand(record), "invalid or future guided payload is read-only")
		check(FileAccess.get_file_as_string(bad_path.path_join("profile.json")) == bad_bytes, "invalid guided payload bytes preserved")

	var blocked_path := scratch.path_join("guided-blocked")
	var blocked := PracticeStore.new(blocked_path)
	DirAccess.make_dir_recursive_absolute(blocked_path.path_join("profile.json"))
	check(not blocked.complete_guided_tutorial("G1"), "guided metadata write failure reported")
	check(blocked.state.guided_tutorial.completed.is_empty() and blocked.guided_resume_lesson() == "G1", "failed write does not claim guided completion")
	DirAccess.remove_absolute(blocked_path.path_join("profile.json"))
	check(blocked.complete_guided_tutorial("G1") and blocked.guided_resume_lesson() == "G2", "guided save retries after repair")
