...
static func normalize(raw: Dictionary) -> Dictionary:
	...
	if ["simple", "medium", "hard", "hell"].has(str(raw.get("difficulty", ""))): out.difficulty = str(raw.difficulty)
...
static func validate(raw: Dictionary) -> bool:
	...
	if not ["simple", "medium", "hard", "hell"].has(str(raw.get("difficulty"))): return false
