...
func _cards(body: Node, title: String, cards: Array, visible_cards: bool = true) -> void:
	...
	if cards.is_empty():
		var empty := _label("尚未发出", 16, host._muted_color())
...
func tutorial_home() -> void:
	var page := _page("新手教程 · 七课入门", "TutorialHomePanel")
	page.body.add_child(_label("每课使用预设局面。完成进度单独保存，重练不计入普通战绩。",18,host._muted_color()))
	for lesson in TutorialController.lessons():
		var done: bool = host.practice_store.state.tutorial_completed.has(lesson.id)
		page.body.add_child(_button("%s  %s%s" % [lesson.id, lesson.title, "  ✓ 已完成" if done else ""], "Lesson_"+lesson.id, func(): open_lesson(lesson.id)))
		page.body.add_child(_label(lesson.summary,16,host._muted_color()))
	page.footer.add_child(_button("返回首页","TutorialHomeBackButton",host._show_menu))
...
func _render_lesson() -> void:
	...
	var page := _page("%s · %s" % [tutorial.lesson_id,step.title],"TutorialLessonPanel")
	...
	if tutorial.lesson_id == "T1":
		_cards(page.body,"你的底牌区域",cards.slice(0,2))
		_cards(page.body,"公共牌区域",cards.slice(2))
		page.body.add_child(_label("座位旁：玩家筹码区域　　桌面中央：底池区域",18,host.COLOR_BRASS))
	elif not cards.is_empty():
		_cards(page.body,"当前示例牌",cards)
	if tutorial.game != null:
		_cards(page.body,"你的底牌",tutorial.game.players[0].hole_cards)
		if not tutorial.game.community_cards.is_empty():
			_cards(page.body,"当前公共牌",tutorial.game.community_cards)
	...
	if tutorial.completed:
		...
		page.body.add_child(_label("课程进度已保存。" if saved else "课程已完成，但进度未保存："+host.practice_store.notice,18,host.COLOR_BRASS))
		if not saved:
			page.footer.add_child(_button("重试保存","TutorialSaveRetry",_render_lesson))
		var index: int = PracticeStore.LESSONS.find(tutorial.lesson_id)
		if index < 6:
			page.footer.add_child(_button("下一课","TutorialNextLesson",func(): open_lesson(PracticeStore.LESSONS[index+1])))
		else:
			page.footer.add_child(_button("进入自由对战","TutorialStartFree",func(): host._show_mode_config("free")))
	page.footer.add_child(_button("重练本课","TutorialRestartButton",func(): open_lesson(tutorial.lesson_id)))
	page.footer.add_child(_button("课程列表","TutorialBackButton",tutorial_home))
...
func history() -> void:
	...
	for match_id in matches:
		...
		page.body.add_child(_label("%s UTC · %s · 本场 %d 条记录" % [first.created_at.replace("T"," "), _mode(first.config.mode), matches[match_id].size()],20,host.COLOR_BRASS))
		for record in matches[match_id]:
			var row := HBoxContainer.new()
			row.add_theme_constant_override("separation",8)
			page.body.add_child(row)
			var view := _button("第 %d 手 · %d 人 · %s · %d 筹码 · %d/%d · 净变化 %+d" % [record.hand_number,int(record.config.ai_count)+1,_difficulty(record.config.difficulty),record.config.initial_stack,record.config.small_blind,record.config.big_blind,record.net_change],"ReplayRecord_"+record.id,func(): open_replay(record,false))
			view.size_flags_horizontal = Control.SIZE_EXPAND_FILL
			row.add_child(view)
			row.add_child(_button("删除","DeleteRecord_"+record.id,func(): _confirm_delete(record.id)))
	_store_notice(page.body)
	page.footer.add_child(_button("返回首页","HistoryBackButton",host._show_menu))

func _confirm_delete(id: String) -> void:
	var popup: PopupPanel = host._show_text_popup("删除这手牌谱？","只删除该手完整回放。累计统计、教程进度和已解锁成就保留。","DeleteReplayPopup")
	popup.get_child(0).add_child(_button("确认删除","ConfirmDeleteReplay",func():
		popup.hide()
...
func _pending_notice(body: Node) -> void:
	if host._pending_records.is_empty() and host._pending_matches.is_empty():
		return
	body.add_child(_label("有尚未保存的记录：%s\n关闭游戏前请重试保存。" % host._practice_save_error,16,host.COLOR_BRASS))
	body.add_child(_button("重试保存","PracticeSaveRetry",func():
		host._retry_practice_saves()
		history()))

func stats() -> void:
	var page := _page("我的统计与成就","StatsPanel")
	...
	_filter(choices,"时间（UTC）","days",["全部时间","最近 7 天","最近 30 天"],[null,7,30])
	_filter(choices,"模式","mode",["全部模式","自由对战","练习对局"],[null,"free","practice"])
	_filter(choices,"辅助","assistance_viewed",["全部辅助来源","独立对局","实际查看过辅助"],[null,false,true])
	_filter(choices,"难度","difficulty",["全部难度","简单","普通","困难"],[null,"simple","medium","hard"])
	_filter(choices,"人数","ai_count",["全部人数","2 人","3 人","4 人","5 人","6 人"],[null,1,2,3,4,5])
	var depths: Array = []
	...
	for depth in depths:
		...
		values.append(depth)
	_filter(choices,"深度","depth",names,values)
	...
	_render_stats()
	page.footer.add_child(_button("指标口径","StatsDefinitions",func(): host._show_text_popup("统计口径","赢手表示独占赢得至少一个主池或边池；平分手另计，两者可以重叠。净盈利手以本手筹码变化大于零计算。未获任何奖池计为输手。\n不同盲注的原始筹码分组显示；BB 是大盲单位。提前离桌不算整场输赢。牌型只统计参与摊牌时的七选五结果。旧历史汇总不能反推牌谱或成就。辅助筛选只看是否真正显示过实时结果，打开不可用面板不算辅助。","StatsDefinitionsPopup")))
	page.footer.add_child(_button("返回首页","StatsBackButton",host._show_menu))
...
func _render_stats() -> void:
	...
	stats_body.add_child(_label("完成 %d 手 · 独占获奖 %d 手 · 平分 %d 手 · 未获奖 %d 手\n净盈利手 %d · 总净变化 %+.2f BB · 最大单手净盈利 %.2f BB" % [stats_value.hands,stats_value.exclusive_win_hands,stats_value.split_hands,stats_value.loss_hands,stats_value.positive_net_hands,stats_value.net_bb,stats_value.max_net_bb]))
	stats_body.add_child(_label("完整结束 %d 场（赢 %d / 输 %d）· 提前离桌 %d 场\n摊牌获奖 %d 手 · 无人争夺获奖 %d 手 · 单场最高筹码 %d · 最高入场倍数 %.2f" % [stats_value.completed_matches,stats_value.won_matches,stats_value.lost_matches,stats_value.left_matches,stats_value.showdown_wins,stats_value.uncontested_wins,stats_value.peak_stack,stats_value.peak_entry_multiple]))
	stats_body.add_child(_label("按盲注分组的净筹码："+_counts(stats_value.net_profit),16,host._muted_color()))
	stats_body.add_child(_label("摊牌形成牌型："+_counts(stats_value.rank_counts)+"\n以牌型获奖："+_counts(stats_value.rank_win_counts)+"\n其中独占："+_counts(stats_value.rank_exclusive_counts)+"\n其中平分："+_counts(stats_value.rank_split_counts),16,host._muted_color()))
	stats_body.add_child(_label("玩家画像 · 当前筛选样本 %d 手\n画像维度和评分尚未定义，暂不生成雷达图。" % stats_value.hands,18,host.COLOR_BRASS))
	var legacy: Dictionary = host.practice_store.state.legacy_stats
	stats_body.add_child(_label("历史汇总（无法追溯、不参与上述筛选）：%d 手 · 胜手 %d · 净筹码 %d" % [legacy.get("total_hands",0),legacy.get("total_win_hands",0),legacy.get("total_net_profit",0)],16,host._muted_color()))
	stats_body.add_child(_label("已解锁成就（全部来源）",20,host.COLOR_BRASS))
	var awards: Dictionary = host.practice_store.state.achievements
	if awards.is_empty():
		stats_body.add_child(_label("尚无成就。完成课程、完整回放或正常对局后解锁。",16))
	for award in awards.values():
		if award is Dictionary:
			stats_body.add_child(_label("✓ %s · %s" % [award.get("name",""),award.get("source","")],16))

func _counts(values: Dictionary) -> String:
	if values.is_empty(): return "暂无数据"
	var text: Array[String] = []
	for key in values: text.append("%s：%s" % [key,str(values[key])])
	return " · ".join(text)

func open_replay(record: Dictionary, from_table: bool) -> void:
	if record.get("frames",[]).is_empty():
		host._show_text_popup("没有完整牌谱","只有已结算的有效手牌可以回放。","ReplayUnavailablePopup")
		return
	...
	replay_visited = {}
	replay_elapsed = 0.0
	var page := _page("第 %d 手回放 · %s · 净变化 %+d" % [record.hand_number,_mode(record.config.mode),record.net_change],"ReplayPanel")
	...
	perspective.text = "全知视角（仅此已结束手牌，包含已弃牌底牌）"
	...
	replay_notice = _label("回放是独立只读记录；不会重新计算胜负或累计战绩。",16,host._muted_color())
	...
	controls.add_child(_button("上一动作","ReplayPrevious",func(): _seek(replay_index-1)))
	controls.add_child(_button("播放","ReplayPlay",func(): replay_playing = true))
	controls.add_child(_button("暂停","ReplayPause",func(): replay_playing = false))
	controls.add_child(_button("下一动作","ReplayNext",func(): _seek(replay_index+1)))
	var stages := {}
	...
	page.footer.add_child(_button("分析说明","ReplayAnalysis",func(): host._show_text_popup("复盘分析尚不可用","本地教练算法尚未定义。全知视角可展示底牌，但未来评价历史决策时只能使用当时可知的信息。当前不提供行动评分或最佳行动建议。","ReplayAnalysisPopup")))
	page.footer.add_child(_button("返回牌桌" if from_table else "返回记录","ReplayCloseButton",_close_replay))
...
func _render_replay_frame() -> void:
	...
	replay_counter.text = "%d / %d · %s · %s" % [replay_index+1,replay.frames.size(),host._stage_label(frame.stage),frame.label]
	_cards(replay_body,"当前公共牌",frame.community_cards)
	for i in range(frame.players.size()):
		...
		_cards(replay_body,"%s · 筹码 %d" % [player.name,player.stack],player.hole_cards,exposed)
		var status: String = {"active":"在局","folded":"已弃牌","all_in":"全下","out":"已出局"}.get(player.status,player.status)
		replay_body.add_child(_label("%s · 本轮投入 %d · 本手投入 %d" % [status,player.current_bet,player.total_bet],14,host._muted_color()))
	replay_body.add_child(_label("底池 %d · 本轮最高下注 %d · 庄位 %d · 小盲位 %d · 大盲位 %d" % [frame.pot,frame.current_bet,frame.button_index,frame.small_blind_player_index,frame.big_blind_player_index],16))
	if frame.stage == "hand_over":
		for refund in replay.refunds:
			replay_body.add_child(_label("未跟注返还：%s %d" % [frame.players[int(refund.player_index)].name,refund.amount],16))
		for pot in replay.pots:
			var names: Array[String] = []
			for payout in pot.get("payouts",[]):
				names.append("%s +%d" % [frame.players[int(payout.player_index)].name,payout.amount])
			replay_body.add_child(_label("奖池 %d：%s" % [pot.amount,"、".join(names)],16))
	if replay_visited.size() == replay.frames.size():
		if host.practice_store.mark_replay_complete(replay.id):
			replay_notice.text = "已完整遍历这手牌谱；回放成就已保存。"
		else:
			replay_notice.text = "已完整回放；成就未保存（牌谱可能尚未成功保存），可在保存后重试回放。"
...
func _mode(mode: String) -> String:
	return {"free":"自由对战","practice":"练习对局","tutorial":"教程"}.get(mode,mode)

func _difficulty(difficulty: String) -> String:
	return {"simple":"简单","medium":"普通","hard":"困难"}.get(difficulty,difficulty)
